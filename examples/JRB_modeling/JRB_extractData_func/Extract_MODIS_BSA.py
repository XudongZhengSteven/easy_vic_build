...
def combine_MODIS_BSA_data(date_period=["2008", "2018"]):
    ...
    for k in Albedo_BSA_months.keys():
        ...
        np.save(
            os.path.join(top, f"combine_MODIS_BSA_JRB_month{k}_{date_period[0]}_{date_period[1]}.npy"),
            Albedo_BSA_month_array,
        )
        np.savetxt(os.path.join(top, f"MODIS_BSA_lat_JRB.txt"), lat_Albedo_BSA_JRB)
        np.savetxt(os.path.join(top, f"MODIS_BSA_lon_JRB.txt"), lon_Albedo_BSA_JRB)


def gapfillingBSA():
    src_home = "E:\\data\\hydrometeorology\\MODIS\\MCD43D51 v061_Black_Sky_Albedo_Shortwave_daily_1km\\JRB\\2008_2018"
    for i in range(1, 13):
        src_path = os.path.join(src_home, f"combine_MODIS_BSA_JRB_month{i}_2008_2018.npy")
        ...
        np.save(
            os.path.join(
                src_home, f"combine_MODIS_BSA_JRB_month{i}_filled_2008_2018.npy"
            ),
            filled_data,
        )
...
def ExtractData(
    ...
    BSA_home = "E:\\data\\hydrometeorology\\MODIS\\MCD43D51 v061_Black_Sky_Albedo_Shortwave_daily_1km\\JRB\\2008_2018"
    ...
    # read lat, lon, res
    BSA_lat = np.loadtxt(os.path.join(BSA_home, f"MODIS_BSA_lat_JRB.txt"))
    BSA_lon = np.loadtxt(os.path.join(BSA_home, f"MODIS_BSA_lon_JRB.txt"))
    ...
    for m in range(1, 13):
        ...
        BSA_month_path = os.path.join(
            BSA_home, f"combine_MODIS_BSA_JRB_month{m}_filled_2008_2018.npy"
        )
