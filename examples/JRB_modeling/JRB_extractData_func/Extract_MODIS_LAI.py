...
def combine_MODIS_LAI_data(reverse_lat=True, rstr=r"A\d{7}", date_period=["2008", "2018"]):
    src_home = "E:\\data\\hydrometeorology\\MODIS\\MOD15A2H v061_LAI_FPAR_8day_500m_2000_02_18_to_Present\\JRB\\data"
    dst_home = "E:\\data\\hydrometeorology\\MODIS\\MOD15A2H v061_LAI_FPAR_8day_500m_2000_02_18_to_Present\\JRB"
    suffix = ".hdf"
    
    src_names = [fn for fn in os.listdir(src_home) if fn.endswith(suffix) and int(date_period[0]) <= int(re.search(rstr, fn)[0][1:5]) <= int(date_period[1])]
    ...
    # save
    with rasterio.open(
        os.path.join(dst_home, f"combine_data_{date_period[0]}_{date_period[1]}_months.tif"),
        "w",
        driver="GTiff",
        height=array_height,
        width=array_width,
...
def reproject_MODIS_LAI_data(date_period=["2008", "2018"]):
    src_home = "E:\\data\\hydrometeorology\\MODIS\\MOD15A2H v061_LAI_FPAR_8day_500m_2000_02_18_to_Present\\JRB"
    dst_home = src_home
    src_path = os.path.join(src_home, f"combine_data_{date_period[0]}_{date_period[1]}_months.tif")
    dst_path = os.path.join(dst_home, f"combine_data_{date_period[0]}_{date_period[1]}_months_reproject.tif")
...
def gapfillingLAI(date_period=["2008", "2018"]):
    src_home = "E:\\data\\hydrometeorology\\MODIS\\MOD15A2H v061_LAI_FPAR_8day_500m_2000_02_18_to_Present\\JRB"
    dst_home = src_home
    src_path = os.path.join(src_home, f"combine_data_{date_period[0]}_{date_period[1]}_months_reproject.tif")
    dst_path = os.path.join(
        dst_home, f"combine_data_{date_period[0]}_{date_period[1]}_months_reproject_filled.tif"
    )
...
def resampleLAI(date_period=["2008", "2018"], scale_factor=1/2):
    src_home = "E:\\data\\hydrometeorology\\MODIS\\MOD15A2H v061_LAI_FPAR_8day_500m_2000_02_18_to_Present\\JRB\\2008_2018"
    dst_home = src_home
    
    src_path = os.path.join(src_home, f"combine_data_{date_period[0]}_{date_period[1]}_months_reproject_filled.tif")
    dst_path = os.path.join(
        dst_home, f"combine_data_{date_period[0]}_{date_period[1]}_months_reproject_filled_resampled_1km.tif"
    )
    ...
    with rasterio.open(src_path) as src:
        ...
        data = src.read(
            ...
            resampling=Resampling.average  # nearest, cubic, average
        )
...
def ExtractData(
    ...
    LAI_home = "E:\\data\\hydrometeorology\\MODIS\\MOD15A2H v061_LAI_FPAR_8day_500m_2000_02_18_to_Present\\JRB\\2008_2018"
    LAI_path = os.path.join(
        LAI_home, "combine_data_2008_2018_months_reproject_filled_resampled_1km.tif"
    )
    ...
    # search grids
    logger.info("searching grids for LAI data... ...")
    
    # level1 > 1km, source data res: 1km (resample from 500m), search LULC grids, then match NDVI grids with LULC grids
    ...
    for i in tqdm(
        ...
        if check_search and i == 0:
            ...
            searched_umd_grids_gdf.plot(
                ax=ax, edgecolor="k", linewidth=1, facecolor="none", alpha=0.5
            )
