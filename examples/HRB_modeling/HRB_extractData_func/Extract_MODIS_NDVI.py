...
def combine_MODIS_NDVI_data(reverse_lat=True, type="mean", rstr=r"A\d{7}", date_period=["2003", "2018"]):
    src_home = (
        "E:\\data\\hydrometeorology\\MODIS\\MOD13A3 v061_NDVI_EVI_monthly_1km\\HRB\\data"
    )
    
    dst_home = (
        "E:\\data\\hydrometeorology\\MODIS\\MOD13A3 v061_NDVI_EVI_monthly_1km\\HRB"
    )
    ...
    combine_func = combine_func[type]
    dst_path = os.path.join(dst_home, f'combine_data_{date_period[0]}_{date_period[1]}_months_{type}.tif')
...
def reproject_MODIS_NDVI_data(type="mean", date_period=["2003", "2018"]):
    src_home = (
        "E:\\data\\hydrometeorology\\MODIS\\MOD13A3 v061_NDVI_EVI_monthly_1km\\HRB"
    )
    dst_home = src_home
    src_path = os.path.join(src_home, f"combine_data_{date_period[0]}_{date_period[1]}_months_{type}.tif")
    dst_path = os.path.join(dst_home, f"combine_data_{date_period[0]}_{date_period[1]}_months_{type}_reproject.tif")
...
def gapfillingNDVI(type="mean", date_period=["2003", "2018"]):
    src_home = (
        "E:\\data\\hydrometeorology\\MODIS\\MOD13A3 v061_NDVI_EVI_monthly_1km\\HRB"
    )
    dst_home = src_home
    
    src_path = os.path.join(src_home, f"combine_data_{date_period[0]}_{date_period[1]}_months_{type}_reproject.tif")
    dst_path = os.path.join(dst_home, f"combine_data_{date_period[0]}_{date_period[1]}_months_{type}_reproject_filled.tif")
...
def ExtractData(
    ...
    NDVI_home = (
        "E:\\data\\hydrometeorology\\MODIS\\MOD13A3 v061_NDVI_EVI_monthly_1km\\HRB\\2003_2018"
    )
    NDVI_path = os.path.join(
        NDVI_home, "combine_data_2003_2018_months_mean_reproject_filled.tif"
    )
    NDVI_max_path = os.path.join(
        NDVI_home, "combine_data_2003_2018_months_max_reproject_filled.tif"
    )
    NDVI_min_path = os.path.join(
        NDVI_home, "combine_data_2003_2018_months_min_reproject_filled.tif"
    )
    ...
    # level1 > 1km, source data res: 1km, search LULC grids, then match NDVI grids with LULC grids
...
if __name__ == "__main__":
    # combine_MODIS_NDVI_data(reverse_lat=True, type="mean")
    # reproject_MODIS_NDVI_data(type="mean")
    # gapfillingNDVI(type="mean")
    pass
