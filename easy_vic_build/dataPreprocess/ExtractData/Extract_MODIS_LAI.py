...
def combine_MODIS_LAI_data(reverse_lat=True):
    ...
    for i in tqdm(range(len(src_names)), desc="loop to Mosaic", colour="g"):
        ...
        if src_year < 2011:
            ...
            with Dataset(src_path, "r", format="NETCDF4_CLASSIC") as dataset:
                ...
                LAI_array[y_index_start: y_index_end + 1, x_index_start: x_index_end + 1] = LAI
    ...
    proj4_LAI = "+proj={} +R={:0.4f} +lon_0={:0.4f} +lat_0={:0.4f} +x_0={:0.4f} +y_0={:0.4f} ".format('sinu', projection_param[0], projection_param[4], projection_param[5], projection_param[6], projection_param[7])
    ...
    # save
    with rasterio.open(os.path.join(dst_home, 'combine_data_2000_2010_months.tif'),
                       'w', driver='GTiff',
                       height=array_height,
                       width=array_width, 
                       count=12,
...
def ExtractData(grid_shp, grid_shp_res=0.125, plot_month=False, save_original=False, check_search=False):
    ...
    LAI_path = os.path.join(LAI_home, "combine_data_2000_2010_months_reproject_filled.tif")
    ...
    with rasterio.open(LAI_path, "r") as src:
        ...
        yindex_start = np.where(LAI_lat >= max(grids_lat) + grid_shp_res)[0][-1]  # large -> small
        ...
        for m in range(1, 13):
            ...
            LAI_month_clip = LAI_month[yindex_start: yindex_end+1, xindex_start: xindex_end+1]
    ...
    for i in tqdm(grid_shp.index, colour="green", desc="loop for each grid to extract LAI"):
        # umd grids
        ...
        for j in range(len(searched_grid_index[0])):
            ...
            searched_grids_index_match = search_grids.search_grids_nearest(dst_lat=[searched_grid_lat], dst_lon=[searched_grid_lon],
                                                                            src_lat=LAI_lat_clip, src_lon=LAI_lon_clip,
            ...
            for m in range(1, 13):
                ...
                searched_match_grid_data = LAI_month_clip[searched_grids_index_match[0][0], searched_grids_index_match[1][0]]
                searched_match_grids_data[m].append(searched_match_grid_data)
                
        # resample and save
        for m in range(1, 13):
            LAI_mean_value = resample.resampleMethod_SimpleAverage(searched_match_grids_data[m], searched_match_grids_lat, searched_match_grids_lon)
        ...
        if check_search and i == 0:
            ...
            searched_match_LAI_grids_gdf = cgdf.createGDF_rectangle_central_coord(searched_match_grids_lon, searched_match_grids_lat, LAI_lat_res)
            ...
            searched_match_LAI_grids_gdf.plot(ax=ax, edgecolor="k", linewidth=1, facecolor="b", alpha=0.5)
    ...
    if plot_month:
        # original, total
        plt.figure()
        show(LAI_months[plot_month], title=f"total_data_LAI_month{plot_month}",
             extent=[LAI_lon[0], LAI_lon[-1],
                     LAI_lat[-1], LAI_lat[0]])
        
        # original, clip
        plt.figure()
        show(LAI_months_clip[plot_month], title=f"total_data_LAI_clip_month{plot_month}",
             extent=[LAI_lon_clip[0], LAI_lon_clip[-1],
                     LAI_lat_clip[-1], LAI_lat_clip[0]])
        
        # readed mean
        fig, ax = plt.subplots()
        grid_shp.plot(f"MODIS_LAI_mean_Value_month{plot_month}", ax=ax, edgecolor="k", linewidth=0.2)
