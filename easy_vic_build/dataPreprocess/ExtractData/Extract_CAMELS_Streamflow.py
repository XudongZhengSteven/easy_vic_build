...
def readStreamflow(hru_id_list=None):
    ...
    if hru_id_list is not None:
        ...
        fpaths = [fpath for fpath in fpaths if int(fpath[fpath.rfind("\\") + 1:fpath.rfind("streamflow")-1]) in hru_id_list]
...
def ExtractData(basin_shp, read_dates=None):
    ...
    fns_streamflow, fpaths_streamflow, usgs_streamflows, streamflow_ids = readStreamflow(basin_shp.hru_id.values.tolist())
...
def checkStreamflowMissing(usgs_streamflow, date_period=["19980101", "20101231"]):
    ...
    usgs_streamflow_date = list(map(lambda i: datetime(*i), zip(usgs_streamflow.loc[:, 1], usgs_streamflow.loc[:, 2], usgs_streamflow.loc[:, 3])))
    usgs_streamflow_date = np.array(usgs_streamflow_date)
    ...
    try:
        ...
        if len(usgs_streamflow.iloc[startIndex:endIndex + 1, :]) < len(date_period_range):
            judgement = False
...
def removeStreamflowMissingfromlists(fns, fpaths, usgs_streamflows, date_period):
    ...
    print(f"count: remove {len(files_Missing)} files, remaining {len(usgs_streamflows)} files")
...
def getremoveStreamflowMissing(date_period=["19980101", "20101231"]):
    # get all streamflow
    fns_streamflow, fpaths_streamflow, usgs_streamflows, streamflow_ids = readStreamflow(hru_id_list=None)
    
    # remove missing
    fns_streamflow_removed_missing, fpaths_streamflow_removed_missing, usgs_streamflows_removed_missing, streamflow_ids_removed_missing, files_Missing = removeStreamflowMissingfromlists(fns_streamflow, fpaths_streamflow, usgs_streamflows, date_period)  # 671 -> 652
    
    # save
    streamflows_dict_original = {"fns": fns_streamflow, "fpaths": fpaths_streamflow, "usgs_streamflows": usgs_streamflows, "streamflow_ids": streamflow_ids}
    streamflows_dict_removed_missing = {"fns": fns_streamflow_removed_missing, "fpaths": fpaths_streamflow_removed_missing, "usgs_streamflows": usgs_streamflows_removed_missing, "streamflow_ids": streamflow_ids_removed_missing, "files_Missing": files_Missing}
