...
def write_agg_netcdf(file_name, agg_data, glob_atts, nc_format, zlib=True,
    ...
    unit_hydrograph_dt = f.createVariable('unit_hydrograph_dt', 'i4', (), **ncvaropts)
    unit_hydrograph_dt.assignValue(int(agg_data['unit_hydrograph_dt']))
    for key, val in iteritems(share.unit_hydrograph_dt):
        if val:
            setattr(unit_hydrograph_dt, key, val.encode())

    timesteps = f.createVariable('timesteps', 'i4', ('timesteps',), **ncvaropts)
    ...
    lon = f.createVariable('lon', 'f8', ('lon',), **ncvaropts)
    for key, val in iteritems(share.lon):
        if val:
            setattr(lon, key, val.encode())

    lat = f.createVariable('lat', 'f8', ('lat',), **ncvaropts)
    ...
    unit_hydrographs = f.createVariable('unit_hydrograph', 'f8',
                                        ('timesteps', 'lat', 'lon',),
...
def write_param_file(file_name,
    ...
    # Timesteps
    timesteps = f.createDimension('timesteps', float(subset_length))
    timesteps = f.createVariable('timesteps', 'f8', ('timesteps',), **ncvaropts)
    timesteps[:] = np.arange(float(subset_length))
    for key, val in iteritems(share.timesteps):
        if val:
            setattr(timesteps, key, val.encode())
    ...
    ftl = f.createVariable('full_time_length', 'i4', (), **ncvaropts)
    ...
    # Subset Length
    sl = f.createVariable('subset_length', 'i4', (), **ncvaropts)
    sl[:] = subset_length
    ...
    # UH timestep
    uh_dt = f.createVariable('unit_hydrograph_dt', 'f8', (), **ncvaropts)
    ...
    olon = f.createVariable('outlet_lon', 'f8', ocoords, **ncvaropts)
    olon[:] = outlet_lon
    ...
    olat = f.createVariable('outlet_lat', 'f8', ocoords, **ncvaropts)
    olat[:] = outlet_lat
    ...
    oxi = f.createVariable('outlet_x_ind', 'i4', ocoords, **ncvaropts)
    oxi[:] = outlet_x_ind
    ...
    oyi = f.createVariable('outlet_y_ind', 'i4', ocoords, **ncvaropts)
    oyi[:] = outlet_y_ind
    ...
    odi = f.createVariable('outlet_decomp_ind', 'i4', ocoords, **ncvaropts)
    ...
    # Outlet Cell Number
    on = f.createVariable('outlet_number', 'i4', ocoords, **ncvaropts)
    on[:] = outlet_number
    ...
    # Outlet Mask
    om = f.createVariable('outlet_mask', 'i4', ocoords, **ncvaropts)
    om[:] = outlet_mask
    ...
    oua = f.createVariable('outlet_upstream_area', 'f8', ocoords,
                           **ncvaropts)
    ...
    oug = f.createVariable('outlet_upstream_gridcells', 'i4', ocoords,
                           **ncvaropts)
    ...
    # Outlet Names
    onm = f.createVariable('outlet_name', 'S1', nocoords)
    onm[:, :] = char_names
    ...
    slon = f.createVariable('source_lon', 'f8', scoords, **ncvaropts)
    slon[:] = source_lon
    ...
    slat = f.createVariable('source_lat', 'f8', scoords, **ncvaropts)
    slat[:] = source_lat
    ...
    sxi = f.createVariable('source_x_ind', 'i4', scoords, **ncvaropts)
    sxi[:] = source_x_ind
    ...
    syi = f.createVariable('source_y_ind', 'i4', scoords, **ncvaropts)
    syi[:] = source_y_ind
    ...
    sdi = f.createVariable('source_decomp_ind', 'i4', scoords, **ncvaropts)
    ...
    sto = f.createVariable('source_time_offset', 'i4', scoords, **ncvaropts)
    ...
    s2o = f.createVariable('source2outlet_ind', 'i4', scoords, **ncvaropts)
    ...
    # Unit Hydrographs
    uhs = f.createVariable('unit_hydrograph', 'f8', uhcords, **ncvaropts)
    uhs[:, :] = unit_hydrograph
    for key, val in iteritems(share.unit_hydrograph):
        if val:
