...
def rout(pour_point, uh_box, fdr_data, fdr_atts, rout_dict):
    ...
    basin['diffusion'] = fdr_data['diffusion'][y_min:y_max, x_min:x_max]  # diffusion
    ...
    t_cell = int(float(rout_dict['CELL_FLOWDAYS']) * float(SECSPERDAY) / float(input_interval))
    t_uh = int(float(rout_dict['BASIN_FLOWDAYS']) * float(SECSPERDAY) / float(input_interval))
    # ---------------------------------------------------------------- #
