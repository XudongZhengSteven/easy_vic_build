...
"""Module ``easy_vic_build.tools.dpc_func.extractData_func.Extract_CONUS_SOIL``."""
...
def set_g_params_CONUS_layer(g_params):
    """
    Run ``set_g_params_CONUS_layer`` data processing routine.
    
    Parameters
    ----------
    g_params : object
        Input value for ``g_params``.
...
def ExtractData(
    ...
    plot_layer : object, optional
        Input value for ``plot_layer``.
...
def get_grids_coord_from_COUNS_SOIL():  # 1km
    # soil data lon, lat
    """
    Run ``get_grids_coord_from_COUNS_SOIL`` data processing routine.
