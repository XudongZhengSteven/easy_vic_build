...
"""Module ``easy_vic_build.tools.dpc_func.extractData_func.Extract_NLDAS``."""
...
def clipNLDAS(dir_name, xmin, xmax, ymin, ymax):
    """
    Run ``clipNLDAS`` data processing routine.
    
    Parameters
    ----------
    dir_name : object
        Input value for ``dir_name``.
    xmin : object
        Input value for ``xmin``.
    xmax : object
        Input value for ``xmax``.
    ymin : object
        Input value for ``ymin``.
    ymax : object
        Input value for ``ymax``.
...
def get_grids_coord_from_NLDAS(read_from_file=True):
    """
    Run ``get_grids_coord_from_NLDAS`` data processing routine.
    
    Parameters
    ----------
    read_from_file : object, optional
        Input value for ``read_from_file``.
