...
def ExtractData(grid_shp, grid_shp_res=0.125, plot_layer=False, check_search=False):
    ...
    for l in range(1, 5):
        ...
        for i in tqdm(
            grid_shp.index,
            colour="green",
            desc=f"loop for each grid to extract ST layer{l}",
        ):
            ...
            searched_grid_data = [
                stls_data[l - 1][searched_grid_index[0][j], searched_grid_index[1][j]]
                for j in range(len(searched_grid_index[0]))
            ...
            if l == 1:
                searched_grid_data_all_layer_mean = [
                    stl_all_layers_mean[
                        searched_grid_index[0][j], searched_grid_index[1][j]
                    ]
                    for j in range(len(searched_grid_index[0]))
                ]  # index: (lat, lon), namely (row, col)
    ...
    if plot_layer:
        ...
        yindex_start = np.where(stl_lat >= max(grids_lat) + grid_shp_res)[0][
            -1
        ]  # large -> small
        ...
        plt.figure()
        show(
            stls_data[plot_layer - 1],
            title=f"total_data_stl{plot_layer}",
            extent=[stl_lon[0], stl_lon[-1], stl_lat[-1], stl_lat[0]],
        )

        # original, clip
        plt.figure()
        data_stl_clip = stls_data[plot_layer - 1][
            yindex_start : yindex_end + 1, xindex_start : xindex_end + 1
        ]
        show(
            data_stl_clip,
            title=f"clip_data_stl{plot_layer}",
            extent=[
                stl_lon_clip[0],
                stl_lon_clip[-1],
                stl_lat_clip[-1],
        ...
        plt.figure()
        data_stl_all_layers_mean_clip = stl_all_layers_mean[
            yindex_start : yindex_end + 1, xindex_start : xindex_end + 1
        ]
        show(
            data_stl_all_layers_mean_clip,
            title=f"clip_data_stl_all_layers_mean",
            extent=[
                stl_lon_clip[0],
                stl_lon_clip[-1],
                stl_lat_clip[-1],
        ...
        fig, ax = plt.subplots()
        grid_shp.plot(
            f"stl{plot_layer}_in_src_grid_Value", ax=ax, edgecolor="k", linewidth=0.2
        )
