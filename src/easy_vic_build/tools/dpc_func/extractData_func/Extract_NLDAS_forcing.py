...
def generate_nldas_filename(dt):
    """
    Run ``generate_nldas_filename`` data processing routine.
...
def process_date_chunk(args):
    """
    Run ``process_date_chunk`` data processing routine.
...
def ExtractData_di(
    ...
    """
    Run ``ExtractData_di`` data processing routine.
    ...
    NLDAS_forcing_home : object
        Input value for ``NLDAS_forcing_home``.
    var_name : object
        Input value for ``var_name``.
    grid_shp : object
        Input value for ``grid_shp``.
    searched_grids_index : object
        Input value for ``searched_grids_index``.
...
def ExtractData(
    ...
    grid_shp : object
        Input value for ``grid_shp``.
    date_period : object
        Input value for ``date_period``.
    search_method : object, optional
        Input value for ``search_method``.
    ...
    N_PROCESS : object, optional
        Input value for ``N_PROCESS``.
